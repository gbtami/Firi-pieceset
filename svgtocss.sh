...
pieces=(+b.svg  b.svg  g.svg  k.svg  +l.svg  l.svg  +n.svg  n.svg  +p.svg  p.svg  +r.svg  r.svg  +s.svg  s.svg
  ...
  a.svg i.svg u.svg c.svg # cannon shogi exclusives
...
#   inkscape --export-type=svg --export-filename=$b $svg
    ...
    # pychess
    cannonPieces=("a" "u" "i" "c") # silver,gold,iron,copper
    emitcss ".cannonshogi piece.${name%.svg}-piece.white.ally"  $B64w >> $CannonShogiCSS
    emitcss ".cannonshogi piece.${name%.svg}-piece.white.enemy" $B64w >> $CannonShogiCSS
    emitcss ".cannonshogi piece.${name%.svg}-piece.black.ally"  $B64b >> $CannonShogiCSS
    emitcss ".cannonshogi piece.${name%.svg}-piece.black.enemy" $B64b >> $CannonShogiCSS
    if [[ ${cannonPieces[*]} =~ ${name%.svg} ]] then
    continue;
    fi
